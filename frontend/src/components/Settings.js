...
const Settings = ({ showNotification }) => {
  ...
  const loadSettings = async () => {
    try {
      ...
      const savedSettings = localStorage.getItem('AegisScan-settings');
      if (savedSettings) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      localStorage.setItem('AegisScan-settings', JSON.stringify(settings));
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = 'AegisScan-settings.json';
      a.click();
